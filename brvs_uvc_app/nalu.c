...
int h264_is_start_code(unsigned char *buf, int offset) {
    if (buf[offset + 0] == 0x00 
        && buf[offset + 1] == 0x00 
        && buf[offset + 2] == 0x00 
        && buf[offset + 3] == 0x01) {
        return 4;
...
#define PARSE_LENGTH_MAX (64)
#define START_CODE_LENGTH_MAX (4)
bool h264_frame_parse(unsigned char *buf, int offset, int length, struct h264_frame_info *info) {
    ...
    if (length < PARSE_LENGTH_MAX + START_CODE_LENGTH_MAX) {
        return false;
    }

    int limit = offset + PARSE_LENGTH_MAX;
    ...
    int skip = 1;
    int i = 0;
    for (i = offset; i < limit; i += skip) {
        start_code_length = h264_is_start_code(buf, i);
        if (start_code_length > 0) {
            type = buf[i + start_code_length];
            switch (type) {
            case 0x41:
            case 0x61:
                if (has_header) {
                    info->header_length = i - info->header_offset;
                }
                info->is_i_frame = false;
                info->data_offset = i;
                info->data_length = offset + length - i;

                // exit loop.
                skip = PARSE_LENGTH_MAX;
                break;
            case 0x65:
                if (has_header) {
                    info->header_length = i - info->header_offset;
                }
                info->is_i_frame = true;
                info->data_offset = i;
                info->data_length = offset + length - i;
            ...
            case 0x68:
                if (!has_header) {
                    has_header = true;
                    info->header_offset = i;
                }
                
                skip = start_code_length + 1;
                break;
            default:
                skip = start_code_length + 1;
                break;
